...
export default function XiaoxiaodewoArticleDetailPage() {
  const params = useParams();
  const id = Number(params.id);
  const article = XIAOXIAODEWO_ARTICLES.find((a) => a.id === id);
  ...
  const nextArticle = XIAOXIAODEWO_ARTICLES.find(a => a.id > id) || XIAOXIAODEWO_ARTICLES[0];
    ...
    <main ref={containerRef} className="relative min-h-screen bg-void text-paper selection:bg-paper selection:text-void pb-32 overflow-x-hidden transition-colors duration-700">
      ...
      <div className="relative z-10 p-6 md:p-24 max-w-7xl mx-auto">
        ...
        <nav className="relative z-50 mb-16 md:mb-24 flex justify-between items-center text-xs font-mono tracking-widest uppercase opacity-60">
          <MagneticButton href="/xiaoxiaodewo/archive">[ 返回影评库 / Back ]</MagneticButton>
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-12">
            <Link href={`/xiaoxiaodewo/archive/${nextArticle.id}`} className="max-w-md group border-l-2 border-transparent hover:border-paper/40 pl-6 transition-all">
