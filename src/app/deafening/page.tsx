...
import { DEAFENING_ARTICLES, DEAFENING_FEATURED_IDS } from "@/src/lib/data";

export default function DeafeningFeaturedPage() {
  const featuredArticles = DEAFENING_ARTICLES.filter(art => DEAFENING_FEATURED_IDS.includes(art.id));
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {featuredArticles.map((art, index) => (
          <motion.div key={art.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6, delay: index * 0.1, ease: "easeOut" }} className="h-full">
            <Link href={`/deafening/archive/${art.id}`} className="block h-full cursor-none">
